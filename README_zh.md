...
| 页面 | 说明 |
|------|------|
| Dashboard | 汇总卡片（总运行数、平均通过率、活跃运行数）和最近运行表格 |
| Configurations | 文件树浏览器 + YAML 编辑器，支持实时校验和快速插入模板 |
| Runs | 启动新运行、查看活跃运行的实时进度、浏览运行历史、对比任意 2 次运行 |
| Results | 逐任务可展开结果，含试验级别的评分、分数、Agent 输出和元数据 |
| Compare | 并排运行对比，含 ECharts 柱状图、指标差异表格和逐任务钻取 |
| Settings | 项目信息（名称、路径、数据库路径） |
